...
def download_audio(
    ...
    Returns (success, stderr) so callers can classify failures.

    force_clients: if provided, override the default player client list.
    """
    ui.progress("Downloading audio")
    ...
    # Build client chain: preferred first, plain YT fallback last
    if force_clients:
        client_chain = force_clients
    elif is_youtube_music:
        client_chain = ['web_music,android', 'ios', 'tv_embedded', 'android,web']
    else:
        client_chain = ['android,web', 'ios', 'tv_embedded', 'web']

    last_stderr = ''

    for client_str in client_chain:
        cmd = get_ytdlp_cmd() + [
            '-x', '--audio-format', fmt,
        ...
        ui.debug(f"Trying player client: {client_str}")
        try:
            ...
            if result.returncode == 0:
                ...
                return True, ''
            ui.debug(f"Client {client_str} failed: {last_stderr[:100]}")
        except subprocess.TimeoutExpired:
            last_stderr = 'Timeout'
            ui.debug(f"Client {client_str} timed out")
        except Exception as e:
            last_stderr = str(e)
            ui.debug(f"Client {client_str} error: {e}")

    ui.error(f"All player clients failed")
...
class FailureCategory(Enum):
    """Category of task failure."""
    UNAVAILABLE = 'unavailable'   # Deleted or private — can never be downloaded
    DOWNLOAD_ERROR = 'error'      # Real failure — worth retrying


# Patterns that definitively mean a video is gone forever
_HARD_UNAVAILABLE_PATTERNS = (
    '[deleted video]',
    '[private video]',
...
# Patterns that mean "unavailable via this client" but plain YouTube may still work
# (geo-block, age-restrict, DRM restriction, temporary block, etc.)
_SOFT_UNAVAILABLE_PATTERNS = (
    'video unavailable',
    'has been removed',
...
def classify_failure(title: str, stderr: str) -> Tuple[FailureCategory, str]:
    """
    Determine if a failure is:
      - UNAVAILABLE  : permanently gone (deleted/private) — never retry
      - SOFT_UNAVAILABLE : blocked via current client but plain YT may work
      - DOWNLOAD_ERROR : generic retriable failure
    Returns (category, human_reason).
    Soft-unavailable is returned as DOWNLOAD_ERROR so the retry path handles it.
    The caller can check the reason string for 'soft_unavailable' if it wants to
    force a plain-YouTube attempt immediately (see process_task).
    """
    combined = (title + ' ' + stderr).lower()

    for pattern in _HARD_UNAVAILABLE_PATTERNS:
        if pattern in combined:
            return FailureCategory.UNAVAILABLE, pattern.strip('[]').title()

    for pattern in _SOFT_UNAVAILABLE_PATTERNS:
        if pattern in combined:
            # Retriable — signal with a prefix so process_task can act on it
            return FailureCategory.DOWNLOAD_ERROR, f'soft_unavailable:{pattern.title()}'

    return FailureCategory.DOWNLOAD_ERROR, stderr[:120].strip() or 'Unknown error'


@dataclass
class FailedTask:
    """A task that failed, with diagnosis."""
    task: 'Task'
...
def process_task(
    ...
    Returns None on success, or a FailedTask describing the failure.
    skip_unavailable: if True, silently skip deleted/private without reporting failure.
    is_retry: if True, use broader player client fallback chain.
    """
    ...
    if not task.download_url.startswith(('http://', 'https://')):
        ...
        return FailedTask(task, FailureCategory.DOWNLOAD_ERROR, 'Invalid URL')
    ...
    if not task.entry:
        ...
        try:
            ...
            if result.stdout.strip():
                ...
                cat, reason = classify_failure('', stderr)
                if cat == FailureCategory.UNAVAILABLE:
                    if skip_unavailable:
                        ui.verbose(f"Skipping unavailable video: {task.download_url}")
                        return None  # Silently skipped — not a failure
                    ui.warning(f"Video unavailable: {reason}")
                    return FailedTask(task, cat, reason)
                ui.error("Could not fetch video info")
                return FailedTask(task, FailureCategory.DOWNLOAD_ERROR, 'Empty info response')
        except Exception as e:
            ui.error(f"Error fetching video info: {e}")
            return FailedTask(task, FailureCategory.DOWNLOAD_ERROR, str(e)[:120])

    # Early unavailability check from entry title
    entry_title = task.entry.get('title', '') or ''
    cat, reason = classify_failure(entry_title, '')
    if cat == FailureCategory.UNAVAILABLE:
        if skip_unavailable:
            ui.verbose(f"Skipping unavailable: {entry_title}")
            return None
        ui.warning(f"Video unavailable: {entry_title}")
        return FailedTask(task, cat, reason)

    # Select metadata
    metadata = select_metadata(
        config, ui, sources,
        task.entry, task.download_url,
        task.metadata_url, task.meta_source,
        no_interactive
    )
    ...
    if not metadata or not metadata.is_complete():
        ...
        if report:
            report.add(entry_title or 'Unknown', 'metadata_failed')
        return FailedTask(task, FailureCategory.DOWNLOAD_ERROR, 'Metadata fetch failed')
    ...
    # Download audio — on retry use a broader client chain ending in plain YT
    retry_clients: Optional[List[str]] = None
    if is_retry:
        if is_youtube_music:
            retry_clients = ['web_music,android', 'ios', 'tv_embedded', 'mweb', 'android,web', 'web']
        else:
            retry_clients = ['android,web', 'ios', 'tv_embedded', 'mweb', 'web']

    dl_ok, dl_stderr = download_audio(
        config, ui, task.download_url, str(output_file),
        is_youtube_music, force_clients=retry_clients
    )

    if not dl_ok:
        cat, reason = classify_failure(entry_title, dl_stderr)

        # Soft-unavailable: the video exists but is blocked via the current
        # client (geo-block, age-restrict, etc.). Try plain YouTube before giving up.
        if reason.startswith('soft_unavailable:') and not is_retry:
            ui.warning(f"Video blocked via current client ({reason[17:]}), trying plain YouTube…")
            plain_clients = ['android,web', 'ios', 'tv_embedded', 'mweb', 'web']
            dl_ok, dl_stderr = download_audio(
                config, ui, task.download_url, str(output_file),
                is_youtube_music=False, force_clients=plain_clients
            )
            if dl_ok:
                ui.success("Plain YouTube fallback succeeded")
            else:
                cat, reason = classify_failure(entry_title, dl_stderr)

        if not dl_ok:
            display_reason = reason[17:] if reason.startswith('soft_unavailable:') else reason
            if cat == FailureCategory.UNAVAILABLE and skip_unavailable:
                ui.verbose(f"Skipping unavailable: {entry_title}")
                return None
            ui.error(f"Download failed ({display_reason[:80]})")
            if report:
                report.add(f"{metadata.artist} - {metadata.title}", 'download_failed')
            return FailedTask(task, cat, display_reason)
...
def export_failures_csv(
    ...
    """Write a CSV of failed tasks so they can be re-fed later."""
    if not failed:
        return None
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    out = output_dir / f'failures_{ts}.csv'
    try:
        with open(out, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['url', 'category', 'reason', 'playlist_folder'])
            for ft in failed:
                writer.writerow([
                    ft.task.download_url,
                    ft.category.value,
                    ft.reason,
                    ft.task.playlist_folder or '',
                ])
        ui.success(f"Failures exported → {out.name}")
        return out
    except Exception as e:
        ui.error(f"Could not write failures CSV: {e}")
        return None
...
CSV format: download_url,metadata_url,meta_source
  (metadata_url and meta_source are optional)
    ...
    parser.add_argument('--skip-unavailable', '-su', action='store_true',
                        help='Silently skip deleted/private videos (do not count as failures)')
    ...
    skip_unavail = args.skip_unavailable
    all_failures: List[FailedTask] = []
    failures_lock = threading.Lock()

    def run_task(
        task: Task, idx: int, total: int,
        report: Optional[ParallelReport],
        is_retry: bool = False
    ) -> Optional[FailedTask]:
        """Wrapper that records FailedTask objects."""
        try:
            ft = process_task(
                config, ui, sources, task, idx, total,
                base_output_dir, no_interactive, report,
                skip_unavailable=skip_unavail,
                is_retry=is_retry,
            )
            return ft
        except Exception as e:
    ...
    if args.mode == 'parallel':
        ...
        for i, task in enumerate(tasks, 1):
            ft = run_task(task, i, len(tasks), None)
            if ft is not None:
                all_failures.append(ft)

    # Split failures by category
    unavailable = [ft for ft in all_failures if ft.category == FailureCategory.UNAVAILABLE]
    real_failures = [ft for ft in all_failures if ft.category == FailureCategory.DOWNLOAD_ERROR]

    # Re-attempt real failures if requested
    still_failed: List[FailedTask] = []
    if args.re_attempt_failed and real_failures:
        ui.separator()
        ui.header(f"Re-attempting {len(real_failures)} Failed Task(s)")

        retry_report = ParallelReport(ui) if args.mode == 'parallel' else None

        if args.mode == 'parallel':
            max_workers = min(len(real_failures), max(1, config.jobs))
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {
                    executor.submit(
                        run_task, ft.task, i, len(real_failures),
                        retry_report, True
                    ): ft
                    for i, ft in enumerate(real_failures, 1)
                }
                for future in as_completed(futures):
                    try:
                        result_ft = future.result()
                        if result_ft is not None:
                            still_failed.append(result_ft)
                    except Exception as e:
                        ui.error(f"Retry error: {e}")
                        still_failed.append(futures[future])
            if retry_report:
                retry_report.print_report()
        else:
            for i, ft in enumerate(real_failures, 1):
                result_ft = run_task(ft.task, i, len(real_failures), None, is_retry=True)
                if result_ft is not None:
                    still_failed.append(result_ft)

        recovered = len(real_failures) - len(still_failed)
        ui.separator()
        ui.success(f"Re-attempt: {recovered}/{len(real_failures)} recovered")
    elif args.re_attempt_failed and not real_failures:
        ui.info("No retriable failures to re-attempt!")
    else:
        still_failed = real_failures

    # ── Final Report ─────────────────────────────────────────────────────────
    ui.separator()
    total_ok = len(tasks) - len(unavailable) - len(still_failed)
    if skip_unavail:
        total_ok = len(tasks) - len(all_failures)  # unavailable were silently skipped

    if not unavailable and not still_failed:
        ui.success(f"All {len(tasks)} task(s) completed successfully")
    else:
        ui.success(f"Completed: {total_ok}/{len(tasks)}")

        if unavailable and not skip_unavail:
            ui.warning(f"Unavailable (deleted/private): {len(unavailable)}")
            for ft in unavailable:
                title = (ft.task.entry or {}).get('title', '') or ft.task.download_url
                ui.info(f"  ⊘ {title}  [{ft.reason}]")

        if still_failed:
            ui.error(f"Failed downloads: {len(still_failed)}")
            for ft in still_failed:
                title = (ft.task.entry or {}).get('title', '') or ft.task.download_url
                ui.error(f"  → {title}")
                ui.verbose(f"     {ft.reason}")

            if not args.re_attempt_failed:
                ui.info("Tip: Use --re-attempt-failed (-rf) to retry these automatically")

    # Export failures CSV (both categories unless unavailable were silently skipped)
    exportable = still_failed + ([] if skip_unavail else unavailable)
    if exportable:
        export_failures_csv(ui, exportable, base_output_dir)
